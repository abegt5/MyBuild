...
export default function AddPostScreen() {
  ...
  const handlePost = async () => {
    try {
      // check if explore or garage
      if (!postType) {
        Alert.alert("Missing Post Type", "Please select 'Garage' or 'Explore'");
        return;
      }
      if (mediaItems.length === 0) {
        Alert.alert("Missing Media", "Please select at least one media item");
        return;
      }
      // Upload media items
      const uploadedUrls = [];
  ...
  return (
    <KeyboardAvoidingView
      ...
      keyboardVerticalOffset={Platform.OS === "ios" ? 10 : 0} // Adjust depending on header/nav height
    >
      <ScrollView
      ...
      >
        <View className="items-center ">
          ...
          <View className="w-full self-center flex-row justify-between items-center bg-[#E5E5EA] rounded-2xl px-1 py-2">
            {/* Garage Button */}
            <TouchableOpacity
            ...
            >
              <Text
                className={`text-xl font-semibold ${
                  postType === "feed" ? "text-primary" : "text-[#8E8E93]"
                }`}
              >
                Garage
              </Text>
            ...
            >
              <Text
                className={`text-xl font-semibold ${
                  postType === "explore" ? "text-primary" : "text-[#8E8E93]"
                }`}
              >
                Explore
              </Text>
          ...
          {!hasChosen ? (
            <TouchableOpacity
              ...
              className={`  h-96 w-full  bg-[#1C1C1E] rounded-lg items-center justify-center m-6 border-dashed border-2 border-gray-500`}
            >
              <View
                style={{
                  width: 50,
          ...
          {/* placement of this might need to be changed so its not accidentally clicked*/}
          <TouchableOpacity
            onPress={handlePost}
            className="w-[90%] bg-[#007AFF] p-4 rounded-lg m-8"
          >
